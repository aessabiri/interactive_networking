...
import { Layers, Network, Router, Play, Pause, RotateCcw, CheckCircle2, Gauge, Mail, ChevronDown, ChevronUp, HelpCircle, FileCode, Terminal, SkipForward, Globe, XCircle, Info, X, Cpu, Hash, Activity, Zap, Laptop, Radio, ShieldCheck, Server } from 'lucide-react';
...
export default function LANModule() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto relative">
      ...
      {/* FLOATING MODAL POPUP FOR ETHERNET FRAME & ARP INSPECTOR (BIG FONTS) */}
      ...
      {/* SIDE-BY-SIDE SWITCH CAM TABLE & HOST ARP CACHE (LEFT) & LOGS (RIGHT) */}
            ...
            <div className="p-3 bg-slate-950 rounded-2xl border border-slate-800 space-y-2">
              ...
              <div className={`p-2 rounded-xl border ${activeStep >= 3 ? 'bg-emerald-950/70 border-emerald-500 text-emerald-200' : 'bg-slate-900/40 border-slate-800 text-slate-600'}`}>
                <div className="flex items-center justify-between font-bold text-[11px]">
                  <span>192.168.1.60</span>
                  <span>{activeStep >= 3 ? '00:66:77:88:99:AA (Dynamic)' : 'No Cache Entry'}</span>
                </div>
              </div>
            </div>
          </div>
